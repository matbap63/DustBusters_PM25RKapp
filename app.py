...
from utils.predictions_utils import get_coordinates, load_pickle, check_within_grid, get_pred
...
core_data = pd.read_csv("/Users/mattye/Downloads/test_app/PM25_sensor_data")
...
st.markdown(
    f"<h3>Predicted PM2.5 (µg/m³):<br><br>{your_prediction}</h4>",
...
if your_prediction < 5.0:
    ...
    st.write('This concentration is considered to be generally safe for both long-term and short-term exposure. ')

elif 5.0 <= your_prediction < 9.0:

    st.write('The Texas Commission on Environmental Quality, according to EPA standards, considers this concentration '
             '(>9.0 µg/m³) to be hazardous at annual concentrations.')
    
elif 9.0 <= your_prediction < 15.0:

    st.write('The World Health organization notes that this concentration (>5.0 µg/m³) may be hazardous ' \
             'if present annually. However, it is not considered hazardous by the Texas Commission on Environmental Quality who, according to EPA standards, recently '\
              'changed their safety thresholds to 9.0 µg/m³' )
    
elif 15.0 <= your_prediction < 35.0:

    st.write('This concentration (>15.0 µg/m³) is considered to be concerning at a 24-hour concentration according to the World Health Organization. '\
             'However, it is not considered so by the Texas Commission on Environmental Quality, who recently changed their standards to >35 µg/m³.')

elif 35.0 <= your_prediction < 50.0:

    st.write('This concentration (>35.0 µg/m³) is considered to be hazardous by the Texas Commission on Environmental Quality.')

elif 50.0 <= your_prediction:

    st.write('This concentration (>50.0 µg/m³) is considered to be hazardous, and prolonged exposure ' \
    'may lead to serious health issues and premature mortality')
...
if hotspots_info:
    ...
    st.write(core_data_sub.tail())

    localG, p_vals = get_localG(core_data_sub, threshold=0.1)
    ...
    hotspots = spots[spots['local_G'] > 1]
    coldspots = spots[spots['local_G'] < 1]
